  ...
  it("should fail when semver doesn't resolve", () => {
    ...
    return fyn
      ...
      .then(() => {
        ...
        expect(error.message).includes("Unable resolve version for package mod-a");
        const message = error.errors.map(e => e.message).join("\n");
        expect(message).includes(
          `Unable to find a version of package mod-a from meta that satisfied semver mod-a@^14.0.0`
        );
      });
  }).timeout(10000);
  ...
  it("should fail when tag doesn't resolve", () => {
    ...
    return fyn
      ...
      .then(() => {
        ...
        const message = error.errors.map(e => e.message).join("\n");
        expect(message).includes(
          `Unable to find a version of package mod-a from meta that satisfied semver mod-a@blah`
        );
      });
  }).timeout(10000);
